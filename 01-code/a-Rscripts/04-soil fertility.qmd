...
test <- subset(soil_fert, sample_date > "2024-08-31" & treatment == "CM")
...
```{r}
#LMM for treatment differences
model <- lmer(bray_p_ppm ~ treatment + (1 | sample_date/block), data = soil_fert)
AIC(model)
summary(model)
...
#ANOVA for sample date differences (because LMM did not meet model assumptions)
aov <- aov(bray_p_ppm ~ as.factor(sample_date), data = soil_fert)
summary(aov)
TukeyHSD(aov)
AIC(aov)
plot(aov)
...
sum_stat_p <- soil_p %>%
              group_by(year) %>%
              summarize(avg_bray_p_ppm = mean(bray_p_ppm, na.rm = TRUE),
                        sd_bray_p_ppm = sd(bray_p_ppm, na.rm = TRUE))


# SI figure:

ggplot(data = subset(soil_fert, month == 5 | month == 10), aes(x = as.factor(sample_date), 
                   y = bray_p_ppm, 
                   fill = treatment)) + 
  geom_boxplot() + 
  theme_bw() + 
  xlab("Sampling Date") + 
  ylab("Bray Phosphorus (ppm)") +
  ylim(285,365) + 
  ...
  scale_x_discrete(labels = c("May 2023", "Oct 2023", "May 2024", "Oct 2024")) + 
  annotate("text", x = 1, y = 328, label = "a") + 
  annotate("text", x = 2, y = 360, label = "b") +
  annotate("text", x = 3, y = 360, label = "c") +
  annotate("text", x = 4, y = 360, label = "d")

#ggsave("20250810_SCM_soil_p.png", width = 6, height = 4)
...
model <- lmer(bray_p_ppm ~ treatment + (1 | month/block), data = soil_fert_23)
AIC(model)
summary(model)
...
```{r}
#LMM comparing treatment and sample dates
model <- lmer(total_c_wt_pct ~ as.factor(sample_date) + (1 | block), data = soil_fert)
AIC(model)
summary(model)
...
emmeans(model, pairwise ~ as.factor(sample_date))
...
```{r}
# average and SD
sum_stat_c <- subset(soil_fert, month == 5 | month == 10) %>%
              group_by(sample_date, treatment) %>%
              summarize(min = min(total_c_wt_pct, na.rm = TRUE),
                        median = median(total_c_wt_pct, na.rm = TRUE),
                        max = max(total_c_wt_pct, na.rm = TRUE),
                        avg_c_pct = mean(total_c_wt_pct, na.rm = TRUE),
                        sd_c_pct = sd(total_c_wt_pct, na.rm = TRUE))
...
ggplot(data = subset(soil_fert, month == 5 | month == 10), aes(x = as.factor(sample_date), 
  ...
  annotate("text", x = 2, y = 4.2, label = "AB") +
  annotate("text", x = 3, y = 4.2, label = "B") +
  annotate("text", x = 4, y = 4.6, label = "C") +
  guides(fill = FALSE)

ggsave("20251013_SCM_soil_c.png", width = 6, height = 4)
...
aov <- aov(bc_c ~ as.factor(sample_date), data = soil_fert)
summary(aov)
TukeyHSD(aov)
AIC(aov)
plot(aov)

aov <- aov(total_c_wt_pct ~ as.factor(sample_date), data = soil_fert)
summary(aov)
TukeyHSD(aov)
AIC(aov)
plot(aov)
...
```{r}
# Testing difference between years
...
# Testing difference between treatments for 2023
...
# Testing differences between treatments for 2024
# here, there's a difference between bare soil and cardboard and between beans and strawberries. I tried with interactions, but the only significant interaction comparing the same types of mulch with different crops or same crops with different mulches was a difference between cardboard for beans and collards
...
sum_stat_bd <- bd %>%
              group_by(year) %>%
              summarize(avg_bd = mean(bulk_density, na.rm = TRUE),
                        sd_bd = sd(bulk_density, na.rm = TRUE))
...
ggplot(bd, aes(x = year, 
  ...
  scale_fill_manual(name = "Mulch Treatment", 
                    labels = c("Bare soil", "Shredded Cardboard", "Straw"), 
                    values = c("#A6CEE3", "#6A3D9A", "#CAB2D6")) +
  scale_x_discrete(labels = c("May 2023", "Oct 2024"))+
  annotate("text", x = 1, y = 1.05, label = "a") + 
  annotate("text", x = 2, y = 1.35, label = "b") +  
  guides(fill = FALSE)

#ggsave("20250810_SCM_soil_bd.png", width = 6, height = 4)
...
bd <- bd %>%
      mutate(first_crop = ifelse(year == "2023" & crop == "AS", "AS",
                          ifelse(year == "2024" & crop == "AS", "FC",
                          ifelse(year == "2024" & crop == "DB", "AS",
                          ifelse(year == "2023" & crop == "FC", "FC",
                                 "DB")))))

#I changed this up from the original, directly comparing plots, and the only significant difference is for year
aov <- aov(bulk_density ~ treatment*first_crop*year, data = bd)
summary(aov)
TukeyHSD(aov)
AIC(aov)
plot(aov)
...
#LMM for the top model: block is not a significant RE
model <- lmer(bulk_density ~ treatment*crop + (1 | block), data = subset(bd, year == "2024"))
AIC(model)
summary(model)
...
```{r}
#KW difference between sample dates because the distributions were not normal. See below for KWs showing treatment effect 
...
```{r}
# average and SD
sum_stat_nitrate <- soil_fert %>%
                group_by(sample_date) %>%
                summarize(min_NO3 = min(NO3_ppm, na.rm = TRUE),
                          median_NO3 = median(NO3_ppm, na.rm = TRUE),
                          max_NO3 = max(NO3_ppm, na.rm = TRUE),
                          avg_NO3 = mean(NO3_ppm, na.rm = TRUE),
                          sd_NO3 = sd(NO3_ppm, na.rm = TRUE))

write.csv(sum_stat_nitrate, here::here("20250812_SCM_nitrate.csv"), row.names=TRUE)

# SI figure:

ggplot(soil_fert, aes(x = as.factor(sample_date), 
                   y = NO3_ppm, 
                   fill = treatment)) + 
  geom_boxplot() + 
  theme_bw() + 
  ...
  scale_fill_manual(name = "Mulch Treatment", 
                    labels = c("Bare soil", "Shredded Cardboard", "Straw"), 
                    values = c("#A6CEE3", "#6A3D9A", "#CAB2D6")) +
  scale_x_discrete(labels = c("Pre-planting May 2023", 
                              "Post-mulch: 3 wks", 
                              "Post-mulch: 6 wks", 
                              "Harvest Oct 2023", 
                              "Pre-planting May 2024", 
                              "Post-mulch: 3 wks",
                              "Post-mulch: 6 wks",
                              "Harvest Oct 2024")) +
  annotate("text", x = 1, y = 61, label = "A") + 
  annotate("text", x = 2, y = 27, label = "BC") +
  annotate("text", x = 3, y = 27, label = "B") +
  annotate("text", x = 4, y = 27, label = "BD") +
  annotate("text", x = 5, y = 27, label = "AC") +
  annotate("text", x = 6, y = 27, label = "DE") +
  annotate("text", x = 7, y = 27, label = "E") +
  annotate("text", x = 8, y = 27, label = "E") 

ggsave("20250916_SCM_soil_nitrate.png", width = 6, height = 4)
...
#significant difference between bare soil and mulches overall. 
...
#Sampling #4: difference between bare soil and cardboard mulch
...
```{r}
# average and SD
sum_stat_NH4 <- soil_fert %>%
                group_by(sample_date) %>%
                summarize(min_NH4 = min(NH4_ppm, na.rm = TRUE),
                          median_NH4 = median(NH4_ppm, na.rm = TRUE),
                          max_NH4 = max(NH4_ppm, na.rm = TRUE),
                          avg_NH4 = mean(NH4_ppm, na.rm = TRUE),
                          sd_NH4 = sd(NH4_ppm, na.rm = TRUE))

write.csv(sum_stat_NH4, here::here("20250812_SCM_ammonium.csv"), row.names=TRUE)
...
ggplot(soil_fert, aes(x = as.factor(sample_date), 
  ...
  scale_x_discrete(labels = c("Pre-planting May 2023", 
                              ...
                              "Post-mulch: 6 wks",
                              "Harvest Oct 2024")) +
  annotate("text", x = 1, y = 11, label = "AB") + 
  annotate("text", x = 2, y = 11, label = "CD") +
  annotate("text", x = 3, y = 11, label = "D") +
  annotate("text", x = 4, y = 11, label = "B") +
  annotate("text", x = 5, y = 11, label = "AB") +
  annotate("text", x = 5.75, y = 7.5, label = "a") +
  annotate("text", x = 6, y = 7.5, label = "b") +
  annotate("text", x = 6, y = 11, label = "AC") +
  annotate("text", x = 6.25, y = 7.5, label = "b") +
  annotate("text", x = 7, y = 11, label = "AB") +
  annotate("text", x = 7.75, y = 7.5, label = "a") +
  annotate("text", x = 8, y = 7.5, label = "b") +
  annotate("text", x = 8, y = 11, label = "AC") +
  annotate("text", x = 8.25, y = 7.5, label = "b") + 
  guides(fill = FALSE)

ggsave("20250916_SCM_soil_ammonium.png", width = 6, height = 4)
...
dunnTest(NH4_ppm ~ treatment, data =soil_fert_23)
...
#LMM: block is not a significant RE
model <- lmer(bc_NH4 ~ treatment + (1 | block), data = subset(soil_fert_24, sample_date == "2024-06-20"))
AIC(model)
summary(model)
...
```{r}
# LMM: block is a significant random effect. Adding treatment increases the AIC by quite a bit
model <- lmer(total_n_wt_pct ~ as.factor(sample_date) + (1 | block), data = soil_fert)
AIC(model)
summary(model)
...
```{r}
# average and SD
sum_stat_n <- soil_fert %>%
              group_by(treatment, sample_date) %>%
              summarize(min = min(total_n_wt_pct, na.rm = TRUE),
                        median = median(total_n_wt_pct, na.rm = TRUE),
                        max = max(total_n_wt_pct, na.rm = TRUE),
                        avg_n = mean(total_n_wt_pct, na.rm = TRUE),
                        sd_n = sd(total_n_wt_pct, na.rm = TRUE))
...
ggplot(data = subset(soil_fert, month == 5 | month == 10), aes(x = as.factor(sample_date), 
  ...
  scale_fill_manual(name = "Mulch Treatment", 
                    ...
                    values = c("#A6CEE3", "#6A3D9A", "#CAB2D6")) +
  scale_x_discrete(labels = c("May 2023", "Oct 2023", "May 2024", "Oct 2024")) +
  annotate("text", x = 1, y = 0.377, label = "a") + 
  annotate("text", x = 2, y = 0.377, label = "b") +
  annotate("text", x = 3, y = 0.377, label = "bc") +
  annotate("text", x = 4, y = 0.377, label = "c") 

ggsave("20250811_SCM_soil_n.png", width = 6, height = 4)
...
aov <- aov(total_n_wt_pct ~ as.factor(sample_date), data = soil_fert)
summary(aov)
TukeyHSD(aov)
AIC(aov)
plot(aov)
...
```{r}
# ANOVA because block was not a significant RE for the LMM
aov <- aov(cn_ratio ~ as.factor(sample_date), data = soil_fert)
summary(aov)
TukeyHSD(aov)
AIC(aov)
plot(aov)

# And KW because I'm not convinced it meets model assumptions
kruskal.test(cn_ratio ~ as.factor(sample_date), data = soil_fert)
dunnTest(cn_ratio ~ as.factor(sample_date), data = soil_fert)
...
```{r}
# average and SD
sum_stat_cn <- soil_fert %>%
               group_by(treatment, sample_date) %>%
               summarize(min = min(cn_ratio, na.rm = TRUE),
                         median = median(cn_ratio, na.rm = TRUE),
                         max = max(cn_ratio, na.rm = TRUE),
                          avg_cn = mean(cn_ratio, na.rm = TRUE),
...
ggplot(data = subset(soil_fert, month == 5 | month == 10), aes(x = as.factor(sample_date), 
  ...
  scale_fill_manual(name = "Mulch Treatment", 
                    ...
                    values = c("#A6CEE3", "#6A3D9A", "#CAB2D6")) +
  scale_x_discrete(labels = c("May 2023", "Oct 2023", "May 2024", "Oct 2024")) +
  annotate("text", x = 1, y = 12.1, label = "a") + 
  annotate("text", x = 2, y = 12.1, label = "ab") +
  annotate("text", x = 3, y = 12.1, label = "b") +
  annotate("text", x = 4, y = 13.4, label = "c") 

ggsave("20250811_SCM_soil_cn_ratio.png", width = 6, height = 4)
