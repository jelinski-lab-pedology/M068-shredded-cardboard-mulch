...
model <- lmer(total_c_wt_pct ~ treatment*as.factor(sample_date) + (1 | block), data = soil_fert)
AIC(model)
summary(model)
...
#ggsave("20250810_SCM_soil_c.png", width = 6, height = 4)
...
ggsave("20250810_SCM_soil_nitrate.png", width = 6, height = 4)
...
ggsave("20250812_SCM_soil_ammonium.png", width = 6, height = 4)
...
sum_stat_n <- soil_fert %>%
              group_by(sample_date) %>%
              summarize(avg_n = mean(total_n_wt_pct, na.rm = TRUE),
...
sum_stat_cn <- soil_fert %>%
               group_by(sample_date) %>%
               summarize(avg_cn = mean(cn_ratio, na.rm = TRUE),
