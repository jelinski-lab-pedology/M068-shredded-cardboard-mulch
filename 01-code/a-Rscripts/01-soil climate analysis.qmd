...
ggplot() + 
  ...
  geom_line(data = subset(soil_vwc_crop_trt, bed_location == "center"), aes(x = week, y = vwc_per_wk, color = treatment)) + 
  facet_wrap(~year + crop)

ggplot() + 
  geom_point(data = subset(soil_vwc, bed_location == "edge"), aes(x = week, y = vwc_percent, color = treatment), size = 0.75) + 
  geom_line(data = subset(soil_vwc_crop_trt, bed_location == "edge"), aes(x = week, y = vwc_per_wk, color = treatment)) + 
  facet_wrap(~year + crop)

ggplot(subset(soil_vwc, year == 2023), aes(x = as.factor(week), y = vwc_percent, fill = treatment)) + geom_boxplot()
ggplot(subset(soil_vwc, year == 2024), aes(x = as.factor(week), y = vwc_percent, fill = treatment)) + geom_boxplot()
...
ggplot() + 
  geom_point(data = soil_vwc, aes(x = week, y = vwc_percent, color = treatment), size = 0.75) + 
  geom_line(data = soil_vwc_trt, aes(x = week, y = vwc_per_wk, color = treatment)) + 
  facet_wrap(~year + bed_location)
...
### Normality
I have tried as many things I can think of for these transformations - boxcox, log, cubic, sqrt, and all for negatively skewed data. And the wild thing is that, visually, it looks like the most normally distributed dataset I've ever gotten to deal with 
```{r}
...
### Linear Mixed Model
I was just messing around, and the best models seem to come when soil moisture is transformed using the sqrt and is subset by bed location, but I wasn't keeping track of AICs or anything. I need to move away from this until I check with Nic about the transformations.
```{r}
