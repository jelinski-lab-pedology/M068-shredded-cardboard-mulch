...
total <- yield %>%
        ...
        mutate_at(c('block', 'plot', 'treatment', 
                  ...
                  as.factor) %>%
        group_by(crop) %>%
        summarize(all_yield = sum(total_g, na.rm = TRUE)) %>%
        ungroup
...
There are 24 potential outliers. Except for a plant where all three readings were high in 1-B on 7/16/2024, others were individually elevated readings. Only one one was lower than the lower bound. 
```{r}
...
Wowza, 73 potential outliers identified. I'm tempted to do the same here, where I adjust the range a little to keep > 10 and < 50
```{r}
...
#### Collards
87 potential outliers! All but one above the upper bound. I feel more comfortable removing that one. 
```{r}
...
```{r}
# average and SD
yield_yr$total_kg_m2 <- ((yield_yr$total_g/30) * (1/0.3048^2) * (1/1000))

sum_stat_yield <- yield_yr %>%
                    group_by(year, crop, treatment) %>%
                    summarize(min_yield = min(total_kg_m2, na.rm = TRUE),
                              median_yield = median(total_kg_m2, na.rm = TRUE),
                              max_yield = max(total_kg_m2, na.rm = TRUE),
                              avg_yield = mean(total_kg_m2, na.rm = TRUE),
                              sd_yield = sd(total_kg_m2, na.rm = TRUE))
...
write.csv(sum_stat_yield, here::here("20250915_SCM_total_yield.csv"), row.names=TRUE)
...
ggplot(yield_yr, aes(x = year, 
  ...
  geom_boxplot() + 
  theme_bw() + 
  ylab(bquote("Total Yield  " (kg/m^2))) +
...
ggsave("20250915_SCM_yield.png", width = 6, height = 4)
